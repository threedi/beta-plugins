...
class ApiConsole3Di:
    ...
    def __init__(self, iface):
        ...
        locale_path = os.path.join(
            self.plugin_dir,
            'i18n',
            'ApiConsole3Di_{}.qm'.format(locale))
    ...
    def unload(self):
        ...
        for action in self.actions:
            self.iface.removePluginMenu(
                self.tr(u'&ApiConsole3Di'),
                action)
            self.iface.removeToolBarIcon(action)

    @staticmethod
    def uuid_information(organisationID,ModelSlug,simStartTime_string,
                         simEndTime_string,ScenarioName,eMail):
        basic_info_call = {"organisation_uuid": organisationID,
    ...
    @staticmethod
    def constant_rain_information(rainIntensity,rainStartTime_string,
                               rainEndTime_string):
        constant_rain_call = {"rain_events": [
                    {"type": "constant", "intensity": rainIntensity, 
                    "active_from": rainStartTime_string, "active_till": 
                    rainEndTime_string}
                ]}
        return constant_rain_call

    @staticmethod
    def design_rain_information(rainevent,rainStartTime_string,
                                rainEndTime_string):
        design_rain_call = {"rain_events": [
                    {"type": "design", "number": rainevent+3, "active_from": 
                    rainStartTime_string, "active_till": rainEndTime_string}
                ]}
        return design_rain_call

    @staticmethod
    def radar_rain_information(radarMultiplier,rainStartTime_string,
                               rainEndTime_string,radarStartTime):
        radar_rain_call = {"rain_events": [
                    {
                        "type": "radar",
    ...
    def store_basic_results(proces_basic_results):
        basic_results_call = {"store_results":
                    {
    ...
    @staticmethod
    def store_damage(proces_basic_results,Cost_type,flood_month,inundation_time,
                     repair_time_infra,repair_time_buildings):
        damage_results_call = {"store_results":
                    {
                        "process_basic_results":
                        proces_basic_results,
                        "damage_estimation": {
    ...
    def run(self):
        ...
        design_rain = ['rainevent 3', 'rainevent 4', 'rainevent 5', 'rainevent 6', 
        'rainevent 7', 'rainevent 8', 'rainevent 9', 'rainevent 10','t100 kort', 
        't250 kort', 't1000 kort','t100 lang','t250 lang','t1000 lang']
        ...
        if result:
            ...
            if rain_type == 'radar':
                ...
                radarStartTime = self.dlg.radarStartTime.dateTime().toString("yyyy-MM-ddThh:mm")
            
            #load rain and simulation start/end time
            simStartTime_string = self.dlg.simStartTime.dateTime().toString("yyyy-MM-ddThh:mm")
            simEndTime_string = self.dlg.simEndTime.dateTime().toString("yyyy-MM-ddThh:mm")
            rainStartTime_string = self.dlg.rainStartTime.dateTime().toString("yyyy-MM-ddThh:mm")
            rainEndTime_string = self.dlg.rainEndTime.dateTime().toString("yyyy-MM-ddThh:mm")
            ...
            #Create api-calls:
            basic_info_call = self.uuid_information(organisationID,ModelSlug,
                              simStartTime_string,simEndTime_string,ScenarioName,eMail)

            if rain_type == 'constant':
                rain_call = self.constant_rain_information(rainIntensity,
                              rainStartTime_string, rainEndTime_string)
            elif rain_type == 'design':
                rain_call = self.design_rain_information(rainevent,
                                       rainStartTime_string,rainEndTime_string)
            elif rain_type == 'radar':
                rain_call = self.radar_rain_information(radarMultiplier,
                       rainStartTime_string, rainEndTime_string,radarStartTime)
            ...
            if proces_basic_results == 'false':
                ...
                processing_call = self.store_damage(proces_basic_results,Cost_type,
                               flood_month, inundation_time, repair_time_infra,
            ...
            QMessageBox.information(None, "status of api-call:", 'received response: ' + str(resp.json()) + '\n\nperformed api-call: ' + str(task)) 