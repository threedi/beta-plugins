...
def clip(raster: gdal.Dataset, vector: gdal.Dataset, layer_name: str = None):
    ...
    if vector.GetLayerCount() > 1:
        if not layer_name:
            raise ValueError("Vector dataset has more than 1 layer, but layer_name is not provided")
        options = gdal.RasterizeOptions(layers=layer_name, bands=[1], burnValues=[nodatavalue], inverse=True)
    else:
        options = gdal.RasterizeOptions(bands=[1], burnValues=[nodatavalue], inverse=True)
    gdal.Rasterize(raster, srcDS=vector, options=options)


if __name__ == "__main__":
    raster_original_fn = Path(__file__).parent / "test" / "data" / "test_raster_original.tif"
